...
class TestCanUseReject:
    ...
    def test_accept_seqlen_zero(self):
    ...
    def test_accept_zero_heads(self):
        # Core SDPA returns empty inputs before backend selection, so the
        # flex gate is intentionally not reached for a zero head count.
        t = torch.randn(1, 0, 64, 16, device=DEVICE, dtype=BF16)
        with flex_only():
            out = F.scaled_dot_product_attention(t, t, t)
