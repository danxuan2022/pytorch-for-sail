...
static inline bool try_launch_ppu_cast_elementwise_1_2(TensorIteratorBase& iter, const func_t& f, array_t data) {
  ...
  // Function-level variable from the original cast_float/cast_double; referenced by the if (ndim == N) checks below
  // (the entry point already routes by iter.ndim(), so it is always true; kept only to preserve the original semantics)
