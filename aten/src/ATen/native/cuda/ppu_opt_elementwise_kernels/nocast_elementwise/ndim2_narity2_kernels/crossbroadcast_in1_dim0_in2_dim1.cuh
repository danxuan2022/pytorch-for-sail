...
// Joint x/y mapping for small float64 square matrices. The original path with
// size0=128, vt=2 has only 64 x vectors and each thread processes 8 rows serially;
// this path hands 4 rows to different lanes while each x lane still reuses its in2 vector, fully preserving the incoming PyTorch functor.
