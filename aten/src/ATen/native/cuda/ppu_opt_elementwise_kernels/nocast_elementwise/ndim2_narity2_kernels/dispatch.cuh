...
//
// Numerical acceptance constraints: after the same functor is inlined into this family's
// kernel skeletons, the compiler's fma contraction decisions may differ from the stock
// legacy/vectorized skeletons (both are legal evaluations under fmad=true), so outputs show
// value-dependent, position-independent bit-level differences; functors with long formula
// chains such as erf/exp (e.g. gelu_backward) amplify intermediate rounding differences in
// cancellation regions (outputs may differ by tens of ULPs). Such ops are accepted in the
// kernel_scan benchmark by per-op ULP tolerances (top-level ulp_tol field in kernel_cases.json; gelu_backward: norm=dy absolute diff <= 1e-6) and are excluded from the BIT_MISMATCH gate.
