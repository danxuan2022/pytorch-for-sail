...
// When size0 is exactly one vector, only a single x thread of the original 128-thread block is active.
// This path preserves unary result reuse: the y==0 lane computes once per z, and the
// 8x8 y/z lanes only parallelize the independent output write-backs.
