...
static inline bool try_launch_ppu_n1_slice_shmem(TensorIteratorBase& iter, const func_t& f, array_t data) {
  ...
  if constexpr (!(std::is_same_v<res_t, arg0_t> &&
    ...
    // For float32, the staging overhead at the 1M/4M tiers is still higher than legacy; from 16M on, all operators gain steadily.
    ...
    // Production kernel A/B: nt=256 beats nt=128 on all tested unary functors for
    // float32 B=128/512, with shared memory usage and grid guards unchanged.
...
static inline bool try_launch_ppu_5_1(TensorIteratorBase& iter, const func_t& f, array_t data) {
  ...
  if constexpr (!(sizeof(res_t) == sizeof(arg0_t) &&
    ...
    // The shared-memory PCB path actively rejects grids with insufficient parallelism.
    // For the same permuted-contiguous layout, this direct tile preserves dynamic dims/strides while avoiding repeated 5D decomposition of d0s.
